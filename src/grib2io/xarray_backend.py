    ...
    if data_model == "nws-viz":
        ...
        for coord in ds.coords:
            if coord == "refDate":
                ...
                if "threshold" in ds.dims:
                    var_key = list(ds.data_vars.keys())[0]
                    prob_types = ["Probability of event below upper limit", "Probability of event above upper limit"]
                    if ds[var_key].attrs["typeOfProbability"] in prob_types:
...
class GribBackendEntrypoint(BackendEntrypoint):
    ...
    def open_dataset(self, filename, *, drop_variables=None, save_index=True, filters: typing.Mapping[str, typing.Any] = {}, data_model=None):
        """
...
def coords_from_cube(cube) -> typing.Dict[str, xr.Variable]:
    ...
    for k in keys:
        if k is not None:
            if len(cube[k]) > 1:
                coords[k] = xr.Variable(dims=k, data=cube[k], attrs={"grib_name": k})
            elif len(cube[k]) == 1:
                coords[k] = xr.Variable(dims=(), data=cube[k][0], attrs={"grib_name": k})
    return coords
...
class OnDiskArray:
    ...
    def __post_init__(self):
        ...
        if len(self.index) == 1:
            ...
            if self.index.index.nlevels == 1:
                self.shape = (len(self.index.index),) + geo_shape
            else:
    ...
    def __getitem__(self, item) -> np.array:
        ...
        if len(index_slicer_inclusive) == 1:
            array_field_shape = (len(index.index),) + self.geo_shape
        elif len(index_slicer_inclusive) > 1:
        ...
        with open(self.file_name, mode="rb") as filehandle:
            for _key, row in index.iterrows():
...
def parse_grib_index(
    ...
    dim_coords = {}  # key=name of dim, value=list of coord names
    attrs = {}
    coord_attrs = {}
    ...
    coord_attrs["refDate"] = {"standard_name": "forecast_reference_time"}
    ...
    coord_attrs["leadTime"] = {"standard_name": "forecast_period"}
    ...
    if pdtn in {6, 10}:
        ...
        coord_attrs["percentileValue"] = {"long_name": "percentile", "units": "percent"}
...
def assign_xr_meta(ds, frames, cube, non_geo_dims, extra_geo, coord_attrs):
        ...
        for name in coord_names:
            if name == dim_name:
                ...
                if ds[dim_name].size == 1:
                    ...
                    coord_data = [df[df.index.get_level_values(f"{dim_name}_ix") == val][name].unique().item() for val in range(ds[dim_name].size)]
    ...
    try:
        ds = ds.assign_coords({"validDate": ds.coords["refDate"] + ds.coords["leadTime"]})
        ds.validDate.attrs["standard_name"] = "time"
        ds.validDate.attrs["long_name"] = "time"
    except Exception as e:
        warnings.warn(f"could not parse validTime: {e}", stacklevel=2)
...
def make_variables(index, f, non_geo_dims, allow_uneven_dims=False):
    ...
    extra_geo = {"latitude": latitude, "longitude": longitude}
...
def interp_nd(a, *, method, grid_def_in, grid_def_out, method_options=None, num_threads=1):
    ...
    a = grib2io.interpolate(a, method, grid_def_in, grid_def_out, method_options=method_options, num_threads=num_threads)
...
def interp_nd_stations(a, *, method, grid_def_in, lats, lons, method_options=None, num_threads=1):
    front_shape = a.shape[:-2]
    a = a.reshape(-1, a.shape[-2], a.shape[-1])
    a = grib2io.interpolate_to_stations(a, method, grid_def_in, lats, lons, method_options=method_options, num_threads=num_threads)
...
class Grib2ioDataSet:
    ...
    def interp_to_stations(self, method, calls, lats, lons, method_options=None, num_threads=1) -> xr.Dataset:
        ...
        da = self._obj.to_array()
        da.attrs["GRIB2IO_section3"] = self._obj[list(self._obj.data_vars)[0]].attrs["GRIB2IO_section3"]
        da = da.grib2io.interp_to_stations(method, calls, lats, lons, method_options=method_options, num_threads=num_threads)
        ds = da.to_dataset(dim="variable")
        return ds
...
class Grib2ioDataArray:
    ...
    def interp(self, method, grid_def_out, method_options=None, num_threads=1) -> xr.DataArray:
        ...
        if da.chunks is None:
            data = interp_nd(
                da.data, method=method, grid_def_in=grid_def_in, grid_def_out=grid_def_out, method_options=method_options, num_threads=num_threads
            )
        else:
            da.shape[:-2]
    ...
    def interp_to_stations(self, method, calls, lats, lons, method_options=None, num_threads=1) -> xr.DataArray:
        ...
        if da.chunks is None:
            data = interp_nd_stations(
                da.data, method=method, grid_def_in=grid_def_in, lats=lats, lons=lons, method_options=method_options, num_threads=num_threads
            )
        else:
            da.shape[:-1]
            data = da.data.map_blocks(
...
def create_datasets_from_df(df, filename, verbose=False) -> typing.Optional[typing.List[xr.Dataset]]:
    """
    ...
    class Grib2ioDataTree:
        ...
        def interp(self, method, grid_def_out, method_options=None, num_threads=1):
            ...
            def process_tree(node, new_parent):
                ...
                if node.ds is not None and node.ds.data_vars:
                    # Interpolate dataset
                    interp_ds = node.ds.grib2io.interp(method, grid_def_out, method_options=method_options, num_threads=num_threads)
