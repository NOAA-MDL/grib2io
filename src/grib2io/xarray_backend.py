...
class Grib2ioDataSet:
    ...
    def interp_to_stations(
        self, method, calls, lats, lons, method_options=None, num_threads=1
    ) -> xr.Dataset:
...
class Grib2ioDataArray:
    ...
    def interp(
        ...
        -------
        xarray.DataArray
            DataArray interpolated to new grid definition.  The attribute
...
def process_probability_groups(target_tree, pdtn_df, filename):
    ...
    -------
    bool
        True if at least one probability group was successfully processed.
    """
    success = False
